...
async function reportViolation(type, severity, description, screenshot) {
  // If browser is offline, skip reporting to avoid noisy network errors
  if (typeof navigator !== 'undefined' && !navigator.onLine) {
    console.warn('Offline — skipping violation report');
    return;
  }
  ...
  try {
    ...
    // Network errors (no response) are common when backend is down or network changed — warn instead of error
    if (!err?.response) {
      console.warn('Failed to report face violation (network):', err?.message ?? err);
    } else {
...
<template>
  <!-- Main Overlay Wrapper (Centered screen overlay when checking OR error) -->
  <div :class="[
    (isChecking || hasError)
      ? 'fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm'
      : 'proctor-camera-widget fixed bottom-4 left-4 z-50 overflow-hidden rounded-2xl border border-white/20 bg-black/40 p-2 shadow-2xl backdrop-blur-md transition-all duration-300 hover:scale-105'
  ]" :style="(isChecking || hasError) ? 'direction: ltr;' : ''" class="transition-all duration-500">
    <!-- Modal Card Container (Only visible & styled as white card when checking OR error) -->
    <div
      :class="[(isChecking || hasError) ? 'bg-white rounded-3xl p-8 max-w-2xl w-full shadow-2xl border border-slate-200 text-center flex flex-col items-center animate-in zoom-in-95 duration-300' : '']">

      <!-- 1. Error Message State (Centered, large fonts, no video stream) -->
      <template v-if="hasError">
        <div
          class="mb-6 text-rose-600 bg-rose-50 w-20 h-20 rounded-full flex items-center justify-center text-4xl mx-auto border-4 border-rose-100">
          <i class="pi pi-exclamation-triangle"></i>
        </div>

        <h3 class="text-2xl sm:text-3xl font-black text-rose-600 uppercase tracking-tight mb-4 leading-tight">
          {{ currentLang === 'ar' ? 'فشل تشغيل كاميرا المراقبة' : 'Camera Access Failure' }}
        </h3>

        <p class="text-slate-700 font-bold leading-relaxed text-base sm:text-lg mb-8 max-w-md">
          {{ currentLang === 'ar'
            ? `لم نتمكن من تشغيل الكاميرا. يرجى التأكد من توصيل الكاميرا بنجاح والسماح للمتصفح بالوصول إليها من إعدادات
          الموقع، ثم قم إعادة تشغيل الصفحة.`
            : `Unable to access the proctoring camera. Please connect your webcam, allow camera permissions in site
          settings, and reload the page.`
          }}
        </p>

        <div
          class="p-4 bg-slate-50 border border-slate-100 rounded-2xl text-[11px] sm:text-xs text-slate-500 font-bold leading-normal uppercase max-w-md mb-8">
          {{ currentLang === 'ar'
            ? 'المتصفح يحتاج إلى إذن الكاميرا لمراقبة نزاهة الامتحان.'
            : 'Browser camera permission is required to verify identity and monitor exam integrity.'
          }}
        </div>

        <button @click="reloadPage"
          class="w-full max-w-sm py-4 bg-slate-900 text-white rounded-2xl font-black uppercase text-xs tracking-widest hover:bg-brand-primary transition-all shadow-xl">
          {{ currentLang === 'ar' ? 'إعادة تحميل الصفحة' : 'Reload Page' }}
        </button>
      </template>

      <!-- 2. Checking or Active Video State (Only if there is NO error) -->
      <template v-else>

        <!-- Header text shown only on checking screen -->
        <template v-if="isChecking">
          <div
            class="mb-4 text-emerald-600 bg-emerald-50 w-16 h-16 rounded-2xl flex items-center justify-center text-3xl mx-auto">
            <i :class="[
              'pi',
              isLoading ? 'pi-spin pi-spinner text-brand-primary' : 'pi-check-circle text-emerald-600'
            ]"></i>
          </div>

          <h3 class="text-2xl font-black text-slate-950 uppercase tracking-tight mb-2">
        ...
        <!-- Single Unified Video Container (Reused during transition from Centered check to bottom widget) -->
        <div :class="[
          isChecking
            ? 'relative w-[320px] h-[240px] md:w-[480px] md:h-[360px] rounded-2xl overflow-hidden bg-slate-950 shadow-inner border border-slate-200 mx-auto'
            : 'relative w-36 h-28 md:w-44 md:h-32 rounded-xl overflow-hidden bg-slate-950 mx-auto'
        ]" class="transition-all duration-500">
          <video ref="videoRef" autoplay muted playsinline class="w-full h-full object-cover scale-x-[-1]"></video>

          <!-- Loading spinner shown during check mode inside feed -->
          <div v-if="isLoading"
          ...
          <!-- Warning message overlay (only in active exam / not checking) -->
        ...
        <!-- Footer text shown only on checking screen -->
        <template v-if="isChecking">
          <div
            class="mb-4 mt-6 p-3 bg-slate-50 border border-slate-100 rounded-xl text-[10px] text-slate-400 font-bold leading-normal uppercase">
