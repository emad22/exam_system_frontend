...
const t = {
    ...
    title: "Exams",
    subtitle: "Create and manage exams and their question rules",
...
const setDefaultExam = async (exam) => {
    const message = t.confirmDefault
        .replace('{title}', exam.title)
        .replace('{category}', exam.category?.name || 'General');
    ...
    try {
        ...
        showAlert(t.defaultError, 'Error', 'error');
    }
};
...
const deleteExam = async (id) => {
    ...
    try {
        ...
        showAlert(t.deleteError, 'Error', 'error');
    }
};
            ...
            <div v-else class="space-y-8 animate-in fade-in slide-in-from-bottom-6 duration-1000 mt-6 px-4 md:px-8 pb-20">
                ...
                <div class="flex flex-col md:flex-row justify-between items-start md:items-center bg-white p-8 rounded-[2rem] border border-slate-100 shadow-sm space-y-6 md:space-y-0 relative overflow-hidden group">
                    ...
                    <div class="flex flex-wrap items-center gap-4 relative z-10">
                        <Button :label="t.createBtn" icon="pi pi-plus" class="px-8 py-3 rounded-2xl bg-brand-primary border-none shadow-lg shadow-rose-100 text-xs font-black tracking-wider uppercase transition-all hover:-translate-y-1 animate-pulse" @click="router.push('/admin/exams/create')" />
                    </div>
                </div>
                    ...
                    <Card class="border border-slate-100 shadow-sm rounded-[2rem] overflow-hidden pb-4 bg-white">
                        <template #content>
                            <DataTable :value="filteredExams" dataKey="id" paginator :rows="10" 
                                ...
                                <Column :header="t.colExamInfo" style="min-width: 350px">
                                    <template #body="{ data }">
                                        <div class="flex items-center gap-4 py-3">
                                            ...
                                            <div class="min-w-0">
                                                 <div class="flex items-center gap-2">
                                                     ...
                                                     <Tag v-if="data.is_default" :value="t.default" severity="success" class="text-[8px] font-black px-2.5 py-0.5 rounded-xl border-none shadow-sm" />
                                                 </div>
                                                 <div class="text-[10px] text-slate-400 font-bold line-clamp-1 italic mt-1">{{ data.description || t.noDesc }}</div>
                                                 ...
                                                 <div class="flex items-center gap-2 mt-3">
                                                     <span class="text-[9px] font-black bg-slate-100 text-slate-500 px-2 py-0.5 rounded-lg border border-slate-200 uppercase tracking-wider">
                                                         {{ data.language?.name || t.universal }}
                                                     </span>
                                ...
                                <Column :header="t.colStructure" style="min-width: 320px">
                                    <template #body="{ data }">
                                            ...
                                            <div v-for="skill in data.skills" :key="skill.id" class="space-y-1">
                                                <div class="flex items-center justify-between border-b border-slate-50 pb-1">
                                                    ...
                                                    <span class="text-[9px] font-black text-slate-400 bg-slate-50 px-2 py-0.5 rounded-lg border border-slate-100">
                                                        {{ t.totalQs.replace('{count}', data.breakdown?.filter(b => b.skill_id === skill.id).reduce((acc, curr) => acc + curr.count, 0) || 0) }}
                                                    </span>
                                                </div>
                                                <div class="flex flex-wrap gap-1 ml-4">
                                                    <div v-for="lvlBreakdown in data.breakdown?.filter(b => b.skill_id === skill.id).sort((a, b) => a.level_id - b.level_id)" 
                                                        ...
                                                        <span class="text-[8px] font-black text-slate-400 me-1.5">{{ t.levelShort.replace('{num}', lvlBreakdown.level_id) }}</span>
                                ...
                                <Column :header="t.colAttempts" style="width: 140px" class="text-center">
                                    <template #body="{ data }">
                                         <div class="flex flex-col items-center">
                                             <div class="font-black text-slate-800 text-base tracking-tighter">{{ data.attempts_count || 0 }}</div>
                                             <div class="text-[8px] font-black text-slate-400 uppercase tracking-widest mt-0.5">{{ t.colAttempts }}</div>
                                         </div>
                                    </template>
                                </Column>
                ...
                <div v-else class="bg-white rounded-[2rem] shadow-sm border border-slate-100 p-24 text-center mt-6 animate-in zoom-in-95 duration-700">
                    ...
                    <h3 class="text-2xl font-black text-slate-800 mb-4 tracking-tight uppercase">{{ t.emptyTelemetry }}</h3>
                    <p class="text-slate-400 font-bold max-w-sm mx-auto mb-8 text-sm leading-relaxed tracking-wide opacity-80">
                        {{ t.emptyExamsText }}
                    </p>
                    <Button :label="t.createFirstBtn" icon="pi-arrow-right" iconPos="right" class="px-10 py-4 bg-brand-primary border-none rounded-2xl shadow-xl shadow-rose-100 font-black text-xs uppercase tracking-wider hover:-translate-y-1 transition-all" @click="router.push('/admin/exams/create')" />
                </div>
            </div>

            <!-- Rules Modal dialog -->
            <Dialog v-model:visible="showRulesModal" :modal="true" :draggable="false" class="w-full max-w-3xl rounded-[2rem] overflow-hidden">
                <template #header>
                    <div class="flex flex-col">
                        <span class="text-xs font-black text-slate-400 uppercase tracking-widest">{{ t.rulesModalTitle }}</span>
                        <span class="text-base font-black text-slate-800 mt-1" v-if="selectedExam">{{ t.rulesHeader.replace('{title}', selectedExam.title) }}</span>
                    </div>
                </template>
                ...
                <div v-if="selectedExam" class="space-y-6 p-4">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                            <div v-for="rule in skill.rules" :key="rule.level_id" class="bg-white p-4 rounded-xl border border-slate-100 space-y-3 shadow-sm">
                                <div class="text-xs font-extrabold text-slate-400 uppercase tracking-widest border-b border-slate-50 pb-1">
                                    {{ t.levelShort.replace('{num}', rule.level_id) }}
                                </div>
                                <div class="flex flex-col space-y-1">
                                    <label class="text-[9px] font-black text-slate-400 uppercase tracking-wider">{{ t.questionsQuantity }}</label>
                    ...
                    <div class="flex justify-end gap-3 pt-4 border-t border-slate-100">
                        <Button :label="t.cancel" text severity="secondary" @click="showRulesModal = false" class="text-xs font-bold rounded-xl" />
                        <Button :label="isSavingRules ? t.saving : t.saveBtn" :loading="isSavingRules" class="bg-brand-primary border-none text-xs font-black px-6 py-2.5 rounded-xl shadow-lg shadow-rose-100" @click="saveRules" />
                    </div>
                </div>
            </Dialog>
        </div>
