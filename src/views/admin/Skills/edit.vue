...
const labels = {
    activeSystem: "Skills",
    title: "Edit Skill",
    subtitle: "Update skill name and short code",
    loading: "Loading skill...",
    fieldName: "Skill Name",
    fieldCode: "Short Code",
    placeholderName: "E.g. Grammar",
    placeholderCode: "E.g. GRAM",
    nameHelp: "This name will be displayed across assessments and reports",
    codeHelp: "Auto-generated from skill name",
    tiersCount: "Active levels:",
    btnCommit: "Save Changes",
    btnDiscard: "Cancel",
...
const editSkill = async () => {
    ...
    try {
        ...
        showAlert(labels.updateSuccess, 'Success', 'success');
        router.push('/admin/skills');
    } catch (err) {
        console.error(err);
        showAlert(err.response?.data?.message || labels.updateFailed, 'Error', 'danger');
    } finally {
          ...
          <div class="premium-card p-8 md:p-12 relative overflow-hidden bg-white rounded-[2rem] shadow-[0_8px_30px_rgba(0,0,0,0.02)] border border-slate-100">
              ...
              <form @submit.prevent="editSkill" class="relative z-10 space-y-10">
                          ...
                          <div class="flex flex-col space-y-2">
                           ...
                           <div class="md:col-span-2 bg-slate-50/50 p-6 rounded-2xl border border-slate-100/50 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
                               <div class="flex items-center gap-3">
                                   ...
                                   <div>
                                       <p class="text-xs font-black text-slate-700">{{ labels.tiersCount }}</p>
                  ...
                  <div class="pt-8 border-t border-slate-100/60 flex flex-col sm:flex-row items-center justify-between gap-4">
                      <Button :label="labels.btnDiscard" outlined severity="secondary" @click="router.push('/admin/skills')"
                          class="w-full sm:w-auto text-[10px] font-black uppercase tracking-widest px-8 rounded-xl" />
                      <Button :label="isSubmitting ? (currentLang === 'ar' ? 'جاري الحفظ...' : 'PERSISTING...') : labels.btnCommit" 
