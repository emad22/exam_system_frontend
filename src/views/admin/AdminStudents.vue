...
<template>
  <AdminLayout>
    <div class="space-y-10 animate-in fade-in slide-in-from-bottom-4 duration-700 pb-20">
        ...
        <div class="flex flex-col md:flex-row justify-between items-start md:items-center space-y-6 md:space-y-0">
            <div>
                 ...
                 <p class="text-[10px] font-black text-slate-300 uppercase tracking-widest mt-1">Lifecycle management for registered entities</p>
            </div>
            <div class="flex items-center space-x-4">
                ...
                <router-link to="/admin/students/create" class="bg-slate-900 text-white font-black text-[10px] tracking-widest uppercase px-8 py-3.5 rounded-xl shadow-lg shadow-slate-200 hover:bg-black active:scale-95 transition-all">
                    Register Entity +
                </router-link>
            </div>
        </div>
                 ...
                 <table class="w-full text-left">
                     ...
                     <tbody class="divide-y divide-slate-50 text-sm">
                         <tr v-for="student in students" :key="student.id" class="hover:bg-slate-50/50 transition-colors group">
                             <td class="px-10 py-6">
                                 <div class="flex items-center space-x-5">
                                      <div class="w-14 h-14 rounded-2xl bg-slate-100 text-slate-300 flex items-center justify-center font-black group-hover:bg-slate-900 group-hover:text-white transition-all transform group-hover:-rotate-3 shadow-sm border border-slate-50">
                                          {{ student.user?.first_name ? student.user.first_name[0] : 'S' }}
                                      </div>
                                      <div>
                                           <div @click="openView(student)" class="font-black text-slate-700 text-lg tracking-tight leading-tight hover:text-indigo-600 cursor-pointer transition-colors">
                                               {{ student.user?.first_name }} {{ student.user?.last_name }}
                                           </div>
                                           <div class="flex items-center space-x-2 mt-1.5">
                                                <span class="text-[10px] font-bold text-slate-300 tracking-wider font-mono">{{ student.user?.email }}</span>
                                                <span class="w-1 h-1 bg-slate-200 rounded-full"></span>
                                                <span class="text-[10px] font-black text-slate-400 uppercase tracking-tighter">{{ student.student_code || 'UNCODED' }}</span>
                                           </div>
                                      </div>
                                 </div>
                             </td>
                             <td class="px-10 py-6">
                                 <div v-if="student.package" class="bg-indigo-50 text-indigo-600 border border-indigo-100 px-3 py-1.5 rounded-lg text-[9px] font-black uppercase tracking-widest inline-block shadow-sm">
                                     {{ student.package.name }}
                                 </div>
                                 <div v-else class="text-[9px] font-black text-slate-300 uppercase tracking-widest pl-2">Custom_Asset</div>
                             </td>
                             <td class="px-10 py-6">
                                 <div class="flex flex-col space-y-1">
                                     <span :class="student.exam_type === 'adult' ? 'text-emerald-500' : 'text-orange-500'" class="text-[10px] font-black uppercase tracking-widest">
                                         {{ student.exam_type }}
                                     </span>
                                     <span v-if="student.not_adaptive" class="text-[8px] font-bold text-amber-400 uppercase tracking-tighter italic">Non-Adaptive</span>
                                 </div>
                             </td>
                             <td class="px-10 py-6">
                                 <span :class="getSourceStyles(student.registration_source)" class="px-3 py-1.5 rounded-lg text-[9px] font-black uppercase tracking-widest border shadow-sm">
                                     {{ student.registration_source }}
                                 </span>
                             </td>
                             <td class="px-10 py-6 text-right">
                                 <div class="flex items-center justify-end space-x-2">
                                     <button @click="openView(student)" class="w-10 h-10 rounded-xl bg-blue-50 text-blue-500 hover:bg-blue-600 hover:text-white transition-all flex items-center justify-center border border-blue-100 shadow-sm" title="View Details">
                                         <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"/><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"/></svg>
                                     </button>
                                     <button @click="openEdit(student)" class="w-10 h-10 rounded-xl bg-amber-50 text-amber-500 hover:bg-amber-600 hover:text-white transition-all flex items-center justify-center border border-amber-100 shadow-sm" title="Edit Profile">
                                         <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/></svg>
                                     </button>
                                     <button @click="deleteStudent(student)" class="w-10 h-10 rounded-xl bg-rose-50 text-rose-500 hover:bg-rose-600 hover:text-white transition-all flex items-center justify-center border border-rose-100 shadow-sm" title="Purge Identity">
                         ...
                         <tr v-if="students.length === 0">
                             <td colspan="5" class="py-32 text-center text-slate-300 font-black uppercase tracking-[0.2em] text-[10px]">Registry Empty. No identities discovered.</td>
                         </tr>
                     </tbody>
                 </table>
            </div>
    ...
    <div v-if="showViewModal" class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/60 backdrop-blur-xl animate-in fade-in duration-300">
        <div class="bg-white rounded-[3.5rem] shadow-2xl w-full max-w-4xl max-h-[90vh] overflow-hidden flex flex-col animate-in zoom-in slide-in-from-bottom-8 duration-500">
            <div class="px-12 py-10 border-b border-slate-50 bg-slate-50/30 flex justify-between items-center shrink-0">
                ...
                <button @click="showViewModal = false" class="w-14 h-14 rounded-full bg-white border border-slate-100 text-slate-400 hover:text-slate-900 flex items-center justify-center transition-all shadow-sm">✕</button>
            </div>
            ...
            <div class="p-12 overflow-y-auto no-scrollbar space-y-12">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-12">
                    <div class="space-y-8">
                        <h4 class="text-[11px] font-black text-indigo-500 uppercase tracking-[0.3em] flex items-center"><span class="w-2 h-2 bg-indigo-500 rounded-full mr-3"></span> Identity Profile</h4>
                    ...
                    <div class="space-y-8">
                        <h4 class="text-[11px] font-black text-emerald-500 uppercase tracking-[0.3em] flex items-center"><span class="w-2 h-2 bg-emerald-500 rounded-full mr-3"></span> Academic Sync</h4>
                        <div class="grid grid-cols-2 gap-6">
    ...
    <div v-if="showEditModal" class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/40 backdrop-blur-md animate-in fade-in duration-300">
        <div class="bg-white rounded-[3rem] shadow-[0_32px_120px_rgba(0,0,0,0.2)] w-full max-w-2xl overflow-hidden animate-in zoom-in duration-300 flex flex-col max-h-[90vh]">
            <div class="px-10 py-8 border-b border-slate-50 bg-slate-50/30 flex justify-between items-center shrink-0">
                <div>
                     <h3 class="text-2xl font-black text-slate-800 tracking-tight">Sync Entity</h3>
                     <p class="text-[10px] font-black text-slate-300 uppercase tracking-widest">Manual identity reconciliation</p>
                </div>
                <button @click="showEditModal = false" class="w-10 h-10 rounded-full bg-white border border-slate-100 text-slate-400 hover:text-red-500 flex items-center justify-center transition-colors shadow-sm">✕</button>
            </div>
            ...
            <div class="p-10 space-y-12 overflow-y-auto no-scrollbar">
                ...
                <div>
                     ...
                     <input v-model="editForm.password" type="password" placeholder="LEAVE BLANK TO RETAIN CURRENT KEY"
            ...
            <div class="p-10 bg-white border-t border-slate-50 flex justify-end space-x-4 shrink-0">
                <button @click="showEditModal = false" class="px-8 py-3 text-[10px] font-black text-slate-400 uppercase tracking-widest hover:text-slate-600 transition">Discard</button>
