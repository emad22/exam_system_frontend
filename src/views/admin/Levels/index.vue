            ...
            <div
                ...
                <div>
                    <h1 class="text-3xl font-black text-slate-800 tracking-tight lowercase first-letter:uppercase">
                        Adaptive Tiers</h1>
                    <p class="text-[10px] font-black text-slate-300 uppercase tracking-widest mt-1">Configure difficulty
                        matrices and mastery thresholds</p>
                </div>
            ...
            <Card v-else class="border border-slate-100 shadow-sm rounded-[2.5rem] overflow-hidden mt-6 bg-white">
                <template #content>
                    <DataTable :value="filteredLevels" rowGroupMode="subheader" groupRowsBy="skill_id"
                        sortField="skill_id" :sortOrder="1" class="p-datatable-sm" responsiveLayout="scroll">

                        <template #header>
                            <div class="flex flex-col md:flex-row justify-between items-center gap-6 p-2 pb-4">
                                <div
                                    class="flex items-center space-x-4 w-full md:w-auto bg-slate-50/50 p-2 rounded-2xl border border-slate-100">
                                    <span
                                        class="text-[9px] font-black text-slate-400 uppercase tracking-widest flex-shrink-0 ml-2">Domain
                                        Filter</span>
                                ...
                                <div class="w-full md:w-80">
                                    <span class="relative">
                                        <i
                                            class="pi pi-search absolute left-4 top-1/2 -translate-y-1/2 text-slate-300 z-10" />
                                        <InputText v-model="searchQuery" placeholder="Filter tier registry..."
                                            class="pl-12 w-full shadow-sm rounded-xl border-slate-100 bg-slate-50/50 focus:bg-white transition-all font-bold" />
                                    </span>
                                </div>
                            </div>
                        </template>
                        ...
                        <template #rowgroupheader="slotProps">
                            <div
                                class="flex items-center space-x-4 py-3 px-6 bg-slate-50/50 backdrop-blur-sm border-y border-slate-100/50">
                                <span
                                    class="text-[10px] font-black text-slate-400 uppercase tracking-[0.3em] italic">Sequence:
                                    {{ getSkill(slotProps.data.skill_id).name }}</span>
                            </div>
                        </template>
                        ...
                        <Column header="Skill Domain" style="width: 220px">
                            <template #body="{ data }">
                                <div class="flex items-center space-x-3">
                                    <div
                                        class="w-8 h-8 rounded-lg bg-slate-100 text-brand-primary flex items-center justify-center text-sm border border-slate-200">
                                        {{ getSkill(data.skill_id).icon }}
                                    </div>
                                    <span class="font-black text-slate-700 uppercase tracking-tight text-[11px]">{{
                                        getSkill(data.skill_id).name }}</span>
                                </div>
                            </template>
                        </Column>
                            ...
                            <template #body="{ data }">
                                <div class="flex flex-col">
                                    <span class="font-black text-slate-700 uppercase tracking-tight text-xs">{{
                                        data.name }}</span>
                                    <span class="text-[9px] text-slate-400 italic">{{ data.instructions ?
                                        data.instructions.substring(0, 40) + '...' : 'No instructions set' }}</span>
                                </div>
                            </template>
                        </Column>
                        ...
                        <Column header="Score Matrix" style="width: 180px" class="text-center">
                            <template #body="{ data }">
                                <div
                                    class="flex items-center justify-center space-x-3 bg-slate-50 p-2 rounded-xl border border-slate-100">
                        ...
                        <Column header="Default Count" style="width: 140px" class="text-center">
                            <template #body="{ data }">
                                <Tag :value="data.default_question_count" severity="info"
                        ...
                        <Column header="Threshold" style="width: 150px">
                            <template #body="{ data }">
                                <div class="flex items-center space-x-3">
                                    ...
                                    <span class="text-[10px] font-black text-slate-600">{{ data.pass_threshold
                                        }}%</span>
                                </div>
                            </template>
                        </Column>
                        ...
                        <template #empty>
                            <div
                                class="p-16 text-center text-slate-300 font-bold uppercase tracking-widest text-xs italic">
                                No adaptive configurations discovered in system registry.</div>
                        </template>
                    </DataTable>
                </template>
            </Card>
