...
const t = {
    ...
    title: "Levels",
    subtitle: "Set difficulty levels and score thresholds for each skill",
    createBtn: "Add Level",
    domainFilter: "Domain Filter",
    allDomains: "All Domains",
    placeholderSearch: "Filter levels...",
    colSequence: "Sequence",
    colSkill: "Skill",
    colDesignation: "Level Name",
    colScoreMatrix: "Score Range",
    colQuestionCount: "Default Count",
    colThreshold: "Threshold",
    colVisibility: "Visibility",
    colActions: "Actions",
    emptyTelemetry: "No levels found.",
    active: "Active",
    inactive: "Inactive",
                ...
                <div class="flex flex-col md:flex-row justify-between items-start md:items-center bg-white p-8 rounded-[2rem] border border-slate-100 shadow-sm space-y-6 md:space-y-0 relative overflow-hidden group">
                 ...
                 <div class="bg-white rounded-[2rem] border border-slate-100 shadow-sm overflow-hidden">
                     <DataTable :value="filteredLevels" rowGroupMode="subheader" groupRowsBy="skill_id"
                         ...
                         <template #rowgroupheader="slotProps">
                             <div class="flex items-center space-x-4 py-3 px-6 bg-slate-50/50 backdrop-blur-sm border-y border-slate-100/50">
                                 <span class="text-[10px] font-black text-slate-400 uppercase tracking-widest italic">
                                     {{ t.colSkill }}: {{ getSkill(slotProps.data.skill_id).name }}
                                 </span>
                             </div>
                         </template>

                         <Column field="level_number" :header="t.colSequence" style="width: 70px" class="pl-6">
                             <template #body="{ data }">
                                 <span class="font-black text-slate-300 italic">#{{ data.level_number }}</span>
                             </template>
                         </Column>

                         <Column :header="t.colSkill" style="width: 220px">
                             <template #body="{ data }">
                         ...
                         <Column field="name" :header="t.colDesignation" style="min-width: 200px">
                             <template #body="{ data }">
                                 <div class="flex flex-col">
                                     <span class="font-black text-slate-700 uppercase tracking-tight text-xs">{{ data.name }}</span>
                                     <span class="text-[10px] text-slate-400 font-bold tracking-wide mt-0.5">{{ data.instructions ? data.instructions.substring(0, 40) + '...' : t.noInstructions }}</span>
                                 </div>
                             </template>
                         </Column>

                         <Column :header="t.colScoreMatrix" style="width: 180px" class="text-center">
                             <template #body="{ data }">
                         ...
                         <Column :header="t.colQuestionCount" style="width: 140px" class="text-center">
                             <template #body="{ data }">
                                 <Tag :value="`${data.default_question_count} ${t.colQuestionCount}`" severity="info"
                                     class="font-black px-4 py-1.5 rounded-xl bg-brand-primary/5 text-brand-primary border border-brand-primary/10 shadow-sm" />
                             </template>
                         </Column>

                         <Column :header="t.colThreshold" style="width: 150px">
                             <template #body="{ data }">
                                 <div class="flex items-center space-x-3">
                         ...
                         <Column :header="t.colVisibility" style="width: 120px" class="text-center">
                             <template #body="{ data }">
