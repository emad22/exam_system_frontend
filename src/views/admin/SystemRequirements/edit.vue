...
onMounted(async () => {
    try {
        const res = await api.get(`/admin/system-requirements/${route.params.id}`);
        form.value = res.data;
    } catch (err) {
        ...
        <div class="space-y-10 animate-in fade-in slide-in-from-bottom-4 duration-700 pb-20 mt-6 px-4">
            ...
            <div class="flex items-center justify-between bg-white p-8 rounded-3xl border border-slate-100 shadow-sm">
                ...
                <div class="hidden md:flex items-center space-x-2 bg-emerald-50 px-4 py-2 rounded-2xl border border-emerald-100">
                    ...
                    <span class="text-[10px] font-black text-emerald-600 uppercase tracking-widest">Live Metadata Sync</span>
                </div>
            </div>
            ...
            <div v-else class="max-w-6xl mx-auto">
                <form @submit.prevent="updateRequirement" class="space-y-8">
                                ...
                                <template #content>
                                    <div class="p-4 space-y-8">
                                        ...
                                        <div class="flex flex-col space-y-2">
                                            ...
                                            <p class="text-[9px] font-bold text-slate-400 uppercase tracking-widest ml-1 italic">Update the primary identifier for this technical prerequisite.</p>
                                        </div>

                                        <div class="flex flex-col space-y-2">
                                            <label class="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1 mb-1">Functional Description</label>
                                            <Textarea v-model="form.description" rows="5" class="w-full rounded-xl bg-slate-50 border-slate-100 focus:bg-white transition-all text-sm font-medium leading-relaxed" placeholder="Detailed technical guidance..." />
                                        </div>
                                    </div>
                                </template>
                            </Card>
