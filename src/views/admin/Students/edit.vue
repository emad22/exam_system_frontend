    ...
    <AdminLayout>
        <div :class="{ 'arabic-theme': currentLang === 'ar' }" :dir="currentLang === 'ar' ? 'rtl' : 'ltr'" class="w-full">
            ...
            <div v-if="loading" class="flex flex-col items-center justify-center py-32 space-y-4">
                <ProgressSpinner />
                <p class="text-xs font-bold text-slate-400 uppercase tracking-widest">{{ t[currentLang].queryLoading }}</p>
            </div>
            ...
            <div v-else class="space-y-8 animate-in fade-in slide-in-from-bottom-6 duration-1000 pb-20 mt-6 px-4">
                ...
                <div class="flex flex-col md:flex-row justify-between items-start md:items-center bg-white p-8 rounded-[2rem] border border-slate-100 shadow-sm space-y-6 md:space-y-0 relative overflow-hidden group">
                    ...
                    <div class="flex items-center gap-3 relative z-10">
                        <div class="flex items-center gap-1.5 bg-emerald-50 px-4 py-2 rounded-xl border border-emerald-100 shadow-sm" v-if="editForm.is_active">
                            <div class="w-2.5 h-2.5 bg-emerald-500 rounded-full animate-pulse"></div>
                            <span class="text-[9px] font-black text-emerald-600 uppercase tracking-widest">{{ t[currentLang].activeLink }}</span>
                        </div>
                        <div class="flex items-center gap-1.5 bg-rose-50 px-4 py-2 rounded-xl border border-rose-100 shadow-sm" v-else>
                            <div class="w-2.5 h-2.5 bg-rose-500 rounded-full"></div>
                            <span class="text-[9px] font-black text-rose-600 uppercase tracking-widest">{{ t[currentLang].inactiveLink }}</span>
                        </div>
                    </div>
                </div>
                ...
                <form @submit.prevent="saveStudent" class="space-y-8 max-w-6xl mx-auto">
                    ...
                    <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
                        ...
                        <div class="lg:col-span-2 space-y-8">
                                ...
                                <template #content>
                                    <div class="p-8 space-y-6">
                                        <div class="flex items-center gap-3 pb-4 border-b border-slate-50">
                                            ...
                                            <h3 class="text-sm font-black text-slate-800 uppercase tracking-wider">{{ t[currentLang].profileIntegrity }}</h3>
                                        </div>
                                        ...
                                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                            <div class="flex flex-col space-y-1.5">
                                                ...
                                                <InputText v-model="editForm.first_name" required class="w-full rounded-xl bg-slate-50 border-slate-100 focus:bg-white transition-all shadow-sm" />
                                            </div>
                                            <div class="flex flex-col space-y-1.5">
                                                <label class="text-[9px] font-black text-slate-400 uppercase tracking-widest ml-1 mr-1">{{ t[currentLang].lastName }}</label>
                                                <InputText v-model="editForm.last_name" required class="w-full rounded-xl bg-slate-50 border-slate-100 focus:bg-white transition-all shadow-sm" />
                                            </div>
                                        </div>
                                        ...
                                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                            <div class="flex flex-col space-y-1.5">
                                                ...
                                                <InputText v-model="editForm.username" required class="w-full rounded-xl bg-slate-50 border-slate-100 focus:bg-white transition-all shadow-sm" />
                                            </div>
                                            <div class="flex flex-col space-y-1.5">
                                                <label class="text-[9px] font-black text-slate-400 uppercase tracking-widest ml-1 mr-1">{{ t[currentLang].email }}</label>
                                                <InputText v-model="editForm.email" type="email" required class="w-full rounded-xl bg-slate-50 border-slate-100 focus:bg-white transition-all shadow-sm" />
                                            </div>
                                        </div>

                                        <div class="flex flex-col space-y-1.5">
                                            <label class="text-[9px] font-black text-slate-400 uppercase tracking-widest ml-1 mr-1">{{ t[currentLang].phone }}</label>
                                            <InputText v-model="editForm.phone" class="w-full rounded-xl bg-slate-50 border-slate-100 shadow-sm" placeholder="+XX-XXXX-XXXX" />
                                        </div>

                                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                            <div class="flex flex-col space-y-1.5">
                                                <label class="text-[9px] font-black text-slate-400 uppercase tracking-widest ml-1 mr-1">{{ t[currentLang].birthDate }}</label>
                                                <DatePicker v-model="editForm.birth_date" dateFormat="yy-mm-dd" showIcon class="w-full flex-1 rounded-xl bg-slate-50 border-slate-100 shadow-sm" inputClass="rounded-xl bg-slate-50 border-slate-100" />
                                            </div>
                                            <div class="flex flex-col space-y-1.5">
                                                <label class="text-[9px] font-black text-slate-400 uppercase tracking-widest ml-1 mr-1">{{ t[currentLang].gender }}</label>
                                                <Select v-model="editForm.gender" :options="[{label:'Male', value:'male'}, {label:'Female', value:'female'}]" optionLabel="label" optionValue="value" class="w-full rounded-xl bg-slate-50 border-slate-100 shadow-sm" />
                                            </div>
                                        </div>
                                    </div>
                                </template>
                            ...
                            <Card class="border border-slate-100 shadow-sm rounded-[2rem] overflow-hidden bg-white animate-in fade-in zoom-in duration-500">
                                <template #content>
                                    <div class="p-8 space-y-6">
                                        <div class="flex items-center justify-between pb-4 border-b border-slate-50">
                                            ...
                                            <div v-else class="flex items-center gap-3">
                                                <div class="w-9 h-9 rounded-xl bg-slate-100 text-slate-400 flex items-center justify-center border border-slate-200 shadow-sm">
                                                    <i class="pi pi-lock text-sm"></i>
                                                </div>
                                                <h3 class="text-sm font-black text-slate-400 uppercase tracking-wider">{{ t[currentLang].fixedModuleSet }}</h3>
                                            </div>
                                            ...
                                            <span v-else
                                                ...
                                                <i class="pi pi-lock mr-1.5 ml-1.5 text-[8px]"></i> {{ t[currentLang].packageControlled }}
                                            </span>
                                        </div>
                        ...
                        <div class="space-y-8">
                                ...
                                <template #content>
                                    <div class="p-8 space-y-6">
                                        ...
                                        <div class="flex flex-col space-y-1.5">
                                            ...
                                            <Select v-model="editForm.partner_id" :options="partners" optionLabel="partner_name" optionValue="id" class="w-full rounded-xl bg-slate-50 border-slate-100 shadow-sm" />
                                        </div>
                                        ...
                                        <div class="flex flex-col space-y-1.5">
                                            ...
                                            <InputText v-model="editForm.student_code" class="w-full rounded-xl bg-slate-50 border-slate-100 shadow-sm" :placeholder="currentLang === 'ar' ? 'مثال: 123456 (اختياري)' : 'e.g. 123456 (Optional)'" />
                                        </div>

                                        <div class="pt-2 space-y-3.5">
                                            <label class="flex items-center cursor-pointer group bg-slate-50/40 p-4 rounded-2xl border border-slate-100 hover:bg-white transition-all shadow-sm">
                                                <Checkbox v-model="editForm.is_active" :binary="true" />
                                                <span class="ml-3.5 mr-3.5 text-xs font-black text-slate-700 uppercase tracking-widest leading-none">{{ t[currentLang].activeEnrollment }}</span>
                                            </label>

                                            <!-- Retry Logic Control -->
                                            <label class="flex items-center cursor-pointer group bg-rose-50/20 p-4 rounded-2xl border border-rose-500/10 hover:bg-white transition-all shadow-sm">
                                                <Checkbox v-model="editForm.allows_retry" :binary="true" />
                                                <div class="ml-3.5 mr-3.5 flex flex-col justify-center">
                                                    <span class="text-xs font-black text-slate-800 uppercase tracking-wider leading-none">{{ t[currentLang].retryToggle }}</span>
                                                    <span class="text-[9px] font-bold text-slate-500 mt-1 leading-none">{{ t[currentLang].retrySubtitle }}</span>
                                                </div>
                                            </label>
                                        </div>
                                    </div>
                                ...
                                <template #content>
                                    <div class="p-8 space-y-6 relative z-10">
                                         <div class="flex justify-between items-center pb-2 border-b border-white/5">
                                            <h3 class="text-[9px] font-black text-rose-300 uppercase tracking-widest">{{ t[currentLang].credentialOverride }}</h3>
                                            <Button icon="pi pi-refresh" text rounded severity="secondary" size="small" @click="editForm.password = generatePassword()" class="text-white hover:bg-white/10" />
                                         </div>
                                         <div class="flex flex-col space-y-2 relative">
                                             <Password v-model="editForm.password" toggleMask :feedback="false" class="w-full"
                                                 ...
                                                 placeholder="••••••" />
                                             <div class="text-[8px] font-bold text-slate-500 uppercase tracking-widest text-center mt-1">{{ t[currentLang].overrideSubtitle }}</div>
                                         </div>
                                    </div>
                                </template>
                            </Card>
                            ...
                            <div class="pt-4">
                                <Button type="submit" :label="t[currentLang].completeBtn" icon="pi pi-check" :loading="isSaving" 
