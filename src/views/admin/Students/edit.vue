        ...
        <div v-else class="space-y-10 animate-in fade-in slide-in-from-bottom-4 duration-700 pb-20 mt-6 px-4">
            ...
            <div class="flex items-center justify-between bg-white p-8 rounded-3xl border border-slate-100 shadow-sm">
                <div class="flex items-center space-x-6">
                    ...
                    <div>
                        <h1 class="text-2xl font-black text-slate-800 tracking-tight lowercase first-letter:uppercase">
                            Sync Identity</h1>
                        <p class="text-[10px] font-bold text-slate-400 uppercase tracking-widest mt-1">Manual Profile
                            Reconciliation</p>
                    </div>
                </div>
                <div class="hidden md:flex items-center space-x-4">
                    ...
                    <div class="flex items-center space-x-2 bg-rose-50 px-4 py-2 rounded-2xl border border-rose-100"
                        v-else>
                        <div class="w-2 h-2 bg-rose-500 rounded-full"></div>
                        <span class="text-[10px] font-black text-rose-600 uppercase tracking-widest">Inactive
                            Link</span>
                    </div>
                </div>
            </div>
            ...
            <form @submit.prevent="saveStudent" class="space-y-8">
                ...
                <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
                    ...
                    <div class="lg:col-span-2 space-y-8">
                            ...
                            <template #content>
                                <div class="p-4 space-y-8">
                                    <div class="flex items-center space-x-3 mb-2">
                                        ...
                                        <h3 class="text-sm font-black text-slate-800 uppercase tracking-wider">Profile
                                            Integrity</h3>
                                    </div>
                                    ...
                                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                        <div class="flex flex-col">
                                            ...
                                            <InputText v-model="editForm.first_name" required
                                                class="w-full rounded-xl bg-slate-50 border-slate-100 focus:bg-white transition-all uppercase" />
                                        </div>
                                        <div class="flex flex-col">
                                            <label
                                    ...
                                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                        <div class="flex flex-col">
                                            <label
                                                class="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2 ml-1">Username</label>
                                        ...
                                        <div class="flex flex-col">
                                            <label
                                                class="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2 ml-1">Email
                                                Address</label>
                        ...
                        <Card
                            class="border border-slate-100 shadow-sm rounded-3xl overflow-hidden animate-in fade-in zoom-in duration-500">
                            <template #content>
                                    ...
                                    <div class="flex items-center justify-between mb-4">
                                        <div v-if="editForm.package_id === 4" class="flex items-center space-x-3">
                                            ...
                                            <h3 class="text-sm font-black text-slate-800 uppercase tracking-wider">
                                                Module Assignment</h3>
                                        </div>
                                        <div v-else class="flex items-center space-x-3">
                                            <div
                                                class="w-8 h-8 rounded-lg bg-slate-100 text-slate-400 flex items-center justify-center">
                                                <i class="pi pi-lock text-xs"></i>
                                            </div>
                                            <h3 class="text-sm font-black text-slate-400 uppercase tracking-wider">Fixed
                                                Module Set</h3>
                                        </div>
                                        <span v-if="editForm.package_id === 4"
                                            class="text-[9px] font-black text-brand-primary uppercase tracking-widest bg-rose-50 px-3 py-1 rounded-lg border border-indigo-100 flex items-center">
                                            <i class="pi pi-unlock mr-2 text-[8px]"></i> Custom Allowed
                                        </span>
                                        <span v-else
                                            class="text-[9px] font-black text-slate-400 uppercase tracking-widest bg-slate-50 px-3 py-1 rounded-lg border border-slate-100 flex items-center">
                    ...
                    <div class="space-y-8">
                        <Card class="border border-slate-100 shadow-sm rounded-3xl overflow-hidden">
                            <template #content>
                                <div class="p-4 space-y-8">
                                    ...
                                    <div class="flex flex-col">
                                        <label
                                            class="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2 ml-1">Assigned
                                            Partner</label>
                                    ...
                                    <div class="flex flex-col">
                                        <label
                                            class="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2 ml-1">Curriculum
                                            Package</label>
                                        <Select v-model="editForm.package_id" :options="packages" optionLabel="name"
                                            optionValue="id" placeholder="Select a package..."
                                            class="w-full rounded-xl bg-slate-50 border-slate-100" />
                                    </div>

                                    <div class="flex flex-col">
                                        <label
                                            class="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2 ml-1">Identification
                                            Code</label>
                                    ...
                                    <div class="pt-2 space-y-3">
                                        <label
                                            class="flex items-center cursor-pointer group bg-slate-50 p-4 rounded-2xl border border-slate-100 hover:bg-white transition-all">
                                            <Checkbox v-model="editForm.is_active" :binary="true" />
                                            <span
                                                class="ml-4 text-xs font-black text-slate-600 uppercase tracking-widest">Active
                                                Enrollment</span>
                                        </label>

                                        <!-- Retry Logic Control -->
                                        <label
                                            class="flex items-center cursor-pointer group bg-rose-50/50 p-4 rounded-2xl border border-brand-primary/10 hover:bg-white transition-all">
                                            <Checkbox v-model="editForm.allows_retry" :binary="true" />
                                            <div class="ml-4 flex flex-col">
                                                <span
                                                    class="text-xs font-black text-slate-800 uppercase tracking-wider">Allow
                                                    Level Retry</span>
                                                <span
                                                    class="text-[9px] font-bold text-slate-500 uppercase mt-0.5">Allows
                                                    second attempt if student fails a level</span>
                                            </div>
                                        </label>
                                    </div>
                                </div>
                        ...
                        <Card
                            class="border border-slate-100 shadow-sm rounded-3xl overflow-hidden bg-slate-900 text-white">
                            <template #content>
                                <div class="p-4 space-y-6">
                                    <div class="flex justify-between items-center">
                                        <h3 class="text-[10px] font-black text-indigo-400 uppercase tracking-[0.2em]">
                                            Credential Override</h3>
                                    ...
                                    <div class="flex flex-col space-y-2 relative">
                                        <Password v-model="editForm.password" toggleMask :feedback="false"
                                            ...
                                            placeholder="••••••" />
                                        <div
                                            class="text-[8px] font-bold text-slate-500 uppercase tracking-widest text-center">
                                            Leave blank to retain current key</div>
                                    </div>
                                </div>
                            </template>
                        </Card>
                        ...
                        <div class="pt-4">
                            <Button type="submit" label="Commit Sync" icon="pi pi-check" :loading="isSaving"
                                class="w-full py-6 rounded-3xl shadow-lg shadow-rose-100 text-sm font-black tracking-widest uppercase" />
                        </div>
                    </div>
                </div>
            </form>
