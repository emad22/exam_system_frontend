    ...
    <div class="max-w-4xl mx-auto pb-20">
        <div class="flex items-center space-x-4 mb-8">
            ...
            <div>
                ...
                <p class="text-gray-500 text-sm mt-1">Manual entry for the question bank.</p>
            </div>
        </div>
        ...
        <form @submit.prevent="saveQuestion" class="space-y-8">
            ...
            <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-8">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        <label class="block text-sm font-bold text-gray-700 mb-2">Points for Correct Answer</label>
                    ...
                    <div>
                        <label class="block text-sm font-bold text-gray-700 mb-2">Group Identifier (Optional)</label>
                        <input v-model="form.group_tag" type="text" placeholder="e.g. listening_set_a" class="w-full bg-gray-50 border border-gray-200 rounded-lg px-4 py-3 focus:outline-none focus:ring-2 focus:ring-indigo-500 transition font-medium">
                        <p class="text-[10px] text-gray-400 mt-1">Use this to group questions together for selective sampling.</p>
                    </div>
                </div>
            </div>
            ...
            <div v-if="!['writing', 'speaking'].includes(form.type)" class="bg-white rounded-2xl shadow-sm border border-gray-100 p-8">
                <div class="flex justify-between items-center mb-6">
                    ...
                    <button v-if="['mcq', 'short_answer'].includes(form.type)" type="button" @click="addOption" class="text-indigo-600 font-bold text-sm hover:text-indigo-800 transition flex items-center">
                        <span class="mr-1 text-xl">+</span> Add {{ form.type === 'short_answer' ? 'Variation' : 'Option' }}
                    </button>
                </div>
                
                <div class="space-y-4">
                    <div v-for="(option, idx) in form.options" :key="idx" class="flex items-center space-x-4 p-4 rounded-xl border border-gray-50 bg-gray-50/30 group">
                        ...
                        <div class="flex-grow">
                            <input v-model="option.option_text" type="text" :disabled="form.type === 'true_false'" :placeholder="form.type === 'short_answer' ? 'Enter expected answer...' : 'Option ' + (idx + 1)" class="w-full bg-transparent border-none focus:ring-0 font-medium text-gray-700 p-0 disabled:text-gray-400">
                        </div>
                        <div v-if="['mcq', 'short_answer'].includes(form.type)" class="flex-shrink-0 opacity-0 group-hover:opacity-100 transition">
                            <button v-if="form.options.length > 1" type="button" @click="removeOption(idx)" class="text-gray-400 hover:text-red-500 transition">✕</button>
                        </div>
                    </div>
                </div>
                
                <div v-if="form.type === 'mcq' || form.type === 'true_false'" class="mt-6 flex items-center text-xs text-info p-3 bg-indigo-50 text-indigo-700 rounded-lg border border-indigo-100">
                    <span class="mr-2 text-lg">💡</span>
                    Click the circle next to an option to mark it as the **correct** answer.
                </div>
                <div v-if="form.type === 'short_answer'" class="mt-6 flex items-center text-xs text-info p-3 bg-amber-50 text-amber-700 rounded-lg border border-amber-100">
                    <span class="mr-2 text-lg">💡</span>
            ...
            <div class="flex justify-end pt-4">
                <button type="submit" :disabled="isSubmitting" class="bg-gradient-to-r from-indigo-600 to-purple-600 text-white font-bold py-4 px-12 rounded-2xl shadow-xl shadow-indigo-100 hover:shadow-indigo-200 transform hover:-translate-y-0.5 transition duration-200 disabled:opacity-50 disabled:cursor-not-allowed">
                    ...
                    {{ isSubmitting ? 'Saving Question...' : 'Add to Question Bank' }}
                </button>
            </div>
        </form>
    </div>
