        ...
        <div class="space-y-10 animate-in fade-in slide-in-from-bottom-4 duration-700 pb-20 mt-6 px-4 md:px-12">
            ...
            <div class="flex items-center justify-between">
                <div class="flex items-center space-x-6">
                    ...
                    <div>
                        ...
                        <p class="text-[10px] font-black text-slate-400 uppercase tracking-widest mt-1">Manual entry for
                            the question bank</p>
                    </div>
                </div>
            </div>
            ...
            <form @submit.prevent="saveQuestion" class="space-y-8">
                    ...
                    <template #content>
                        <div class="p-4 space-y-6">
                            <h3 class="text-lg font-bold text-slate-800 mb-6 flex items-center">
                                <span
                                    class="w-8 h-8 bg-rose-50 text-brand-primary rounded-lg flex items-center justify-center mr-3 text-sm font-black">1</span>
                                Basic Configuration
                            </h3>
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                                ...
                                <div class="flex flex-col ">
                                    <label class="block text-xs font-bold text-slate-500 mb-2 pl-2">Exam</label>
                                    <Select v-model="form.exam_id" :options="exams" optionLabel="title" optionValue="id"
                                        placeholder="Select exam" class="w-full rounded-xl shadow-sm" />
                                </div>
                                ...
                                <div class="flex flex-col">
                                    ...
                                    <Select v-model="form.skill_id" :options="skills" optionLabel="name"
                                        optionValue="id" placeholder="Select a skill"
                                        class="w-full rounded-xl shadow-sm" />
                                </div>
                                <div class="flex flex-col">
                                    <label class="block text-xs font-bold text-slate-500 mb-2 pl-2">Question
                                        Type</label>
                                    <Select v-model="form.type" @change="handleTypeChange"
                                        :options="[{ label: 'Multiple Choice (MCQ)', value: 'mcq' }, { label: 'True / False', value: 'true_false' }, { label: 'Short Answer (Fill in Blank)', value: 'short_answer' }, { label: 'Writing Prompt', value: 'writing' }, { label: 'Speaking Prompt', value: 'speaking' }]"
                                        optionLabel="label" optionValue="value" class="w-full rounded-xl shadow-sm" />
                                </div>
                                <div class="flex flex-col">
                                    <label class="block text-xs font-bold text-slate-500 mb-3 pl-2">Difficulty Level ({{
                                        form.difficulty_level }})</label>
                                    <Slider v-model="form.difficulty_level" :min="1" :max="9" :step="1"
                                        class="w-full mb-3" />
                                    <div
                                ...
                                <div class="flex flex-col">
                                    <label class="block text-xs font-bold text-slate-500 mb-2 pl-2">Points for Correct
                                        Answer</label>
                                    <InputNumber v-model="form.points" :min="1" showButtons
                                        class="w-full rounded-xl shadow-sm" />
                                </div>

                                <div class="flex flex-col">
                                    <label class="text-xs font-bold mb-2">Media</label>
                                    <input type="file" @change="e => form.media = e.target.files[0]" />
                                </div>
                            </div>
                        </div>
                    </template>
                ...
                <Card class="border border-slate-100 shadow-sm rounded-[2rem]">
                    <template #content>
                        <div class="p-4">
                            <h3 class="text-lg font-bold text-slate-800 mb-6 flex items-center">
                                <span
                                    class="w-8 h-8 bg-rose-50 text-brand-primary rounded-lg flex items-center justify-center mr-3 text-sm font-black">2</span>
                                Question Content
                            </h3>
                            ...
                            <div class="flex flex-col">
                                ...
                                <Textarea v-model="form.content" required rows="4"
                                    placeholder="Enter the question text here..." class="w-full rounded-2xl shadow-sm"
                                    autoResize />
                            </div>
                        </div>
                    </template>
                ...
                <Card v-if="!['writing', 'speaking'].includes(form.type)"
                    class="border border-slate-100 shadow-sm rounded-[2rem]">
                    <template #content>
                        <div class="p-4">
                            <div class="flex justify-between items-center mb-6">
                                <h3 class="text-lg font-bold text-slate-800 flex items-center">
                                    <span
                                        class="w-8 h-8 bg-rose-50 text-brand-primary rounded-lg flex items-center justify-center mr-3 text-sm font-black">3</span>
                                    {{ form.type === 'short_answer' ? 'Correct Variations' : 'Answer Options' }}
                                </h3>
                                <Button v-if="['mcq', 'short_answer'].includes(form.type)" type="button"
                                    @click="addOption" icon="pi pi-plus"
                                    :label="'Add ' + (form.type === 'short_answer' ? 'Variation' : 'Option')" text
                                    size="small" rounded />
                            </div>

                            <div class="space-y-4">
                                <div v-for="(option, idx) in form.options" :key="idx"
                                    class="flex items-center space-x-4 p-4 rounded-xl border bg-slate-50/50 group transition-all"
                                    ...
                                    <div class="flex-grow">
                                        <InputText v-model="option.option_text" :disabled="form.type === 'true_false'"
                                            :placeholder="form.type === 'short_answer' ? 'Enter expected answer...' : 'Option ' + (idx + 1)"
                                            class="w-full bg-white shadow-sm rounded-xl font-bold text-slate-700" />
                                    </div>
                                    <div v-if="['mcq', 'short_answer'].includes(form.type)"
                                        class="flex-shrink-0 opacity-0 group-hover:opacity-100 transition">
                                        <Button v-if="form.options.length > 1" type="button" @click="removeOption(idx)"
                                            icon="pi pi-times" severity="danger" text rounded />
                                    </div>
                                </div>
                            </div>

                            <div v-if="form.type === 'mcq' || form.type === 'true_false'"
                                class="mt-6 flex items-center text-xs p-4 bg-rose-50 text-brand-secondary rounded-xl border border-indigo-100 font-bold tracking-wide">
                            ...
                            <div v-if="form.type === 'short_answer'"
                                class="mt-6 flex items-center text-xs p-4 bg-emerald-50 text-emerald-700 rounded-xl border border-emerald-100 font-bold tracking-wide">
                                <i class="pi pi-info-circle mr-3 text-lg"></i>
                ...
                <div class="flex justify-end pt-4 pb-10">
                    <Button type="submit" :loading="isSubmitting"
                        :label="isSubmitting ? 'Saving Question...' : 'Add to Question Bank'" icon="pi pi-check"
                        size="large" class="shadow-xl rounded-2xl px-10 font-bold tracking-widest uppercase" />
                </div>
            </form>
        </div>
    </AdminLayout>
