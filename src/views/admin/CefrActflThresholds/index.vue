  ...
  <AdminLayout>
    ...
    <div class="w-full space-y-8 animate-in fade-in slide-in-from-bottom-6 duration-700 mt-6 px-4 md:px-8 pb-24">
      ...
      <div class="flex flex-col md:flex-row justify-between items-start md:items-center bg-white p-8 rounded-[2rem] border border-slate-100 shadow-sm relative overflow-hidden group">
        <div class="absolute right-0 top-0 w-64 h-64 bg-violet-50/20 rounded-full -translate-y-1/2 translate-x-1/2 blur-2xl group-hover:bg-violet-100/30 transition-all duration-1000"></div>
        ...
        <div class="relative z-10 space-y-1.5">
          ...
          <p class="text-xs font-bold text-slate-400 max-w-xl leading-relaxed">
            Configure score-to-level mapping rules. Changes take effect immediately on new certificate generation (cache clears automatically).
          </p>
        </div>
      ...
      <div class="grid grid-cols-2 md:grid-cols-4 gap-5">
        <div v-for="card in [
          ...
          <div class="flex items-center justify-between mb-3">
            <div class="w-9 h-9 rounded-xl flex items-center justify-center text-white shadow-sm group-hover:rotate-6 transition-all duration-300" :class="card.color">
      ...
      <div class="bg-white rounded-[2rem] border border-slate-100 shadow-sm overflow-hidden">
        ...
        <div class="flex flex-col md:flex-row items-start md:items-center justify-between gap-4 px-8 pt-7 pb-5 border-b border-slate-50">
          <div class="flex flex-col gap-4 w-full md:w-auto">
            <!-- Skill Group Pills -->
            <div class="flex items-center gap-2">
              <button v-for="g in [{ label: '⚡ Core Skills', value: 'core' }, { label: '🎙️ Productive Skills', value: 'productive' }]"
                      :key="g.value"
                      @click="activeTab = g.value"
            ...
            <!-- Framework Pills -->
            <div class="flex items-center gap-2">
              <button v-for="f in [{ label: 'CEFR Framework', value: 'cefr', color: 'violet' }, { label: 'ACTFL Framework', value: 'actfl', color: 'blue' }]"
                      :key="f.value"
                      @click="activeFramework = f.value"
                      class="px-5 py-2 rounded-xl text-xs font-extrabold uppercase tracking-wider transition-all duration-200 border"
        ...
        <div v-else class="overflow-x-auto">
          <DataTable :value="filteredRows" class="p-datatable-sm" responsiveLayout="scroll" stripedRows>
            ...
            <Column header="MIN SCORE / POINTS" style="width:200px">
              <template #body="{ data }">
                <div class="flex items-center gap-2">
                  <span class="px-3 py-1.5 rounded-xl text-xs font-black border"
                        :class="getScoreColor(data.min_score)">
                    ≥ {{ data.min_score }}
                  </span>
            ...
            <Column header="FRAMEWORK" style="width:140px">
              <template #body="{ data }">
                <Tag :value="data.framework.toUpperCase()"
                     :severity="data.framework === 'cefr' ? 'info' : 'warn'"
      ...
      <!-- ── Score Scale Guide ───────────────────────────────────────────── -->
      <div class="bg-gradient-to-br from-slate-50 to-slate-100/60 rounded-2xl border border-slate-200 p-6">
        <div class="flex items-center gap-2 mb-4">
          <i class="pi pi-info-circle text-slate-400"></i>
          <span class="text-xs font-extrabold text-slate-500 uppercase tracking-wider">Scoring Scale Guide</span>
        </div>
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4 text-xs text-slate-600">
          <div class="space-y-2">
            <p class="font-extrabold text-slate-700">Core Skills (Listening / Reading / Structure)</p>
            <p>Score percentage is converted to <strong>/900 points</strong> before comparing against <code class="bg-white px-1.5 py-0.5 rounded border border-slate-200 font-mono">min_score</code>.</p>
            <p class="text-slate-400">Example: 82.5% score → 742 points, matches threshold ≥ 700 → <em>C1</em></p>
          </div>
          <div class="space-y-2">
            <p class="font-extrabold text-slate-700">Productive Skills (Writing / Speaking)</p>
            <p>Raw score percentage is compared directly against <code class="bg-white px-1.5 py-0.5 rounded border border-slate-200 font-mono">min_score</code>.</p>
            <p class="text-slate-400">Example: 72.0% score, threshold ≥ 70 → <em>Advanced</em></p>
          </div>
        </div>
        <p class="mt-4 text-[10px] font-bold text-slate-400 uppercase tracking-wider">
          ⚡ Thresholds are evaluated top-down (highest min_score first). The first match wins. Inactive rows are skipped.
        </p>
      </div>
    ...
    <Dialog v-model:visible="showDialog" modal
            :header="editingRow ? 'Edit Threshold' : 'Add Threshold'"
            :style="{ width: '480px' }"
      ...
      <div class="space-y-5 py-2">
        ...
        <div class="space-y-1.5">
          ...
          <InputText v-model="form.level_label" placeholder="e.g. C1, Advanced High, B2.1"
                     class="w-full text-sm" maxlength="50" />
      ...
      <template #footer>
        <div class="flex items-center justify-end gap-3 pt-2">
          ...
          <Button :label="editingRow ? 'Save Changes' : 'Create'" icon="pi pi-check"
    ...
    <!-- ── Bulk Replace Dialog ─────────────────────────────────────────────── -->
    <Dialog v-model:visible="showBulkDialog" modal header="Bulk Replace Thresholds"
            :style="{ width: '580px' }"
            class="rounded-2xl overflow-hidden">
      <div class="space-y-5 py-2">
        ...
        <div class="p-4 bg-amber-50 rounded-xl border border-amber-200 flex gap-3">
          ...
          <p class="text-xs font-bold text-amber-700">
            This will <strong>DELETE ALL existing thresholds</strong> for the selected group/framework and replace them with the new list. Use with caution.
          </p>
        </div>
        ...
        <div class="grid grid-cols-2 gap-4">
          <div class="space-y-1.5">
            <label class="text-[10px] font-extrabold text-slate-500 uppercase tracking-widest">Skill Group</label>
            <Select v-model="bulkGroup" :options="groupOptions" optionLabel="label" optionValue="value" class="w-full text-sm" />
          </div>
          <div class="space-y-1.5">
            <label class="text-[10px] font-extrabold text-slate-500 uppercase tracking-widest">Framework</label>
            <Select v-model="bulkFramework" :options="frameworkOptions" optionLabel="label" optionValue="value" class="w-full text-sm" />
          </div>
        </div>
      ...
      <template #footer>
        <div class="flex items-center justify-end gap-3 pt-2">
          <Button label="Cancel" severity="secondary" outlined @click="showBulkDialog = false"
                  class="text-xs font-extrabold uppercase tracking-wider rounded-xl px-6" />
          <Button label="Replace All" icon="pi pi-upload" :loading="bulkSaving" @click="saveBulk"
                  severity="danger"
                  class="text-xs font-extrabold uppercase tracking-wider rounded-xl px-6" />
        </div>
      </template>
