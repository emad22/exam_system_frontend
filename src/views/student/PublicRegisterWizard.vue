    ...
    <div class="min-h-screen bg-slate-50 flex items-center justify-center p-6 font-sans">
        <div class="w-full max-w-xl space-y-10 animate-in fade-in slide-in-from-bottom-8 duration-700">
            ...
            <div class="text-center space-y-4">
                <div class="w-16 h-16 bg-brand-primary rounded-3xl flex items-center justify-center mx-auto shadow-xl shadow-brand-primary/20 transform -rotate-6">
                ...
                <p class="text-[10px] font-black text-slate-400 uppercase tracking-widest leading-loose">Join the Arab Academy’s Elite Assessment Network</p>
            </div>
            ...
            <div class="bg-white rounded-[3rem] shadow-2xl shadow-slate-200/50 border border-slate-100 p-10 md:p-16 relative overflow-hidden">
                ...
                <div v-if="step < 4" class="absolute top-0 left-0 w-full h-1.5 bg-slate-50 flex">
                    <div :style="{ width: (step * 33.33) + '%' }" class="h-full bg-brand-primary transition-all duration-700 ease-out shadow-[0_0_15px_rgba(157,29,35,0.3)]"></div>
                </div>
                ...
                <form @submit.prevent="handleRegister" class="space-y-10">
                    ...
                    <div v-if="step === 1" class="space-y-8 animate-in slide-in-from-right-4 duration-500">
                        ...
                        <div class="grid grid-cols-2 gap-6">
                            <div class="space-y-3">
                                ...
                                <input v-model="form.first_name" type="text" required class="w-full bg-slate-50 border border-slate-100 rounded-2xl p-5 text-sm font-bold focus:ring-4 focus:ring-rose-50 focus:border-brand-primary/20 transition-all outline-none" placeholder="ALI">
                            </div>
                            <div class="space-y-3">
                                <label class="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-4">Family Name</label>
                                <input v-model="form.last_name" type="text" required class="w-full bg-slate-50 border border-slate-100 rounded-2xl p-5 text-sm font-bold focus:ring-4 focus:ring-rose-50 focus:border-brand-primary/20 transition-all outline-none" placeholder="DOE">
                            </div>
                        </div>

                        <div class="space-y-3">
                            <label class="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-4">Contact Phone</label>
                            <input v-model="form.phone" type="tel" class="w-full bg-slate-50 border border-slate-100 rounded-2xl p-5 text-sm font-bold focus:ring-4 focus:ring-rose-50 focus:border-brand-primary/20 transition-all outline-none" placeholder="+20 123 456 789">
                        </div>

                        <div class="pt-6">
                             <button type="button" @click="nextStep" class="w-full bg-brand-primary text-white font-black py-5 rounded-2xl shadow-xl shadow-brand-primary/20 hover:bg-brand-primary/90 transition-all text-xs uppercase tracking-[0.3em]">Continue ➜</button>
                        </div>
                    </div>
                    ...
                    <div v-if="step === 2" class="space-y-8 animate-in slide-in-from-right-4 duration-500">
                        ...
                        <div class="space-y-6">
                            <div class="space-y-3">
                                ...
                                <div class="grid grid-cols-2 gap-4">
                                    <button v-for="cat in categories" :key="cat.id"
                                        ...
                                        :class="form.exam_category_id === cat.id ? 'bg-brand-primary text-white shadow-xl shadow-brand-primary/20' : 'bg-slate-50 text-slate-400 border border-slate-100'" 
                            ...
                            <div class="space-y-3">
                                ...
                                <select v-model="form.gender" class="w-full bg-slate-50 border border-slate-100 rounded-2xl p-5 text-xs font-black uppercase tracking-widest outline-none focus:ring-4 focus:ring-rose-50 transition-all">
                        ...
                        <div class="pt-6 flex space-x-4">
                             <button type="button" @click="prevStep" class="w-1/3 bg-slate-50 text-slate-400 font-black py-5 rounded-2xl transition-all text-[10px] uppercase tracking-widest border border-slate-100">Back</button>
                             <button type="button" @click="nextStep" class="w-2/3 bg-brand-primary text-white font-black py-5 rounded-2xl shadow-xl shadow-brand-primary/20 transition-all text-xs uppercase tracking-[0.3em]">Continue ➜</button>
                        </div>
                    </div>
                    ...
                    <div v-if="step === 3" class="space-y-8 animate-in slide-in-from-right-4 duration-500">
                        ...
                        <div class="space-y-6">
                            ...
                            <div class="space-y-3">
                                ...
                                <input v-model="form.password" type="password" required class="w-full bg-slate-50 border border-slate-100 rounded-2xl p-5 text-sm font-bold focus:ring-4 focus:ring-emerald-50 transition-all outline-none" placeholder="••••••••">
                            </div>
                            <div class="space-y-3">
                                <label class="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-4">Confirm Secret</label>
                                <input v-model="form.password_confirmation" type="password" required class="w-full bg-slate-50 border border-slate-100 rounded-2xl p-5 text-sm font-bold focus:ring-4 focus:ring-emerald-50 transition-all outline-none" placeholder="••••••••">
                            </div>
                        </div>

                        <div class="pt-6 flex space-x-4">
                             <button type="button" @click="prevStep" :disabled="isSubmitting" class="w-1/3 bg-slate-50 text-slate-400 font-black py-5 rounded-2xl transition-all text-[10px] uppercase tracking-widest border border-slate-100">Back</button>
                             <button type="submit" :disabled="isSubmitting" class="w-2/3 bg-brand-primary text-white font-black py-5 rounded-2xl shadow-xl shadow-brand-primary/20 transition-all text-xs uppercase tracking-[0.3em]">
                    ...
                    <div v-if="step === 4" class="text-center space-y-10 py-10 animate-in zoom-in-95 duration-700">
                         ...
                         <div class="space-y-4">
                             ...
                             <p class="text-[10px] font-black text-slate-400 uppercase tracking-widest leading-loose">Identity successfully provisioned. Redirecting to your secure assessment dashboard...</p>
                         </div>
                         <div class="pt-4">
            ...
            <div class="text-center">
                 <router-link to="/login" class="text-[10px] font-black text-slate-400 uppercase tracking-widest hover:text-brand-primary transition-colors">Already have an identity? Access here</router-link>
            </div>
        </div>
    </div>
</template>
