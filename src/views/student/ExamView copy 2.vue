...
const pollSessionStatus = async () => {
    ...
    try {
        const res = await api.get(`/proctoring/session/${sessionId}`)
        ...
        if (status === 'ended' || status === 'cancelled') {
            ...
            try {
                await router.push('/skill-selection')
            } catch (err) {
            ...
            // Don't end the session — it stays paused so the student can resume via a new skill selection
            isIntentionallyLeaving.value = true
            try {
                await router.push('/skill-selection')
            } catch (err) {
        ...
        if (isVerified) {
            try {
                ...
                const response = await api.post('/proctoring/session/initiate', {
                    ...
                    session_id: savedSessionId ? parseInt(savedSessionId) : null
                });
...
onBeforeRouteLeave((to, from) => {
    ...
    if (proctoringSessionId.value && skillId) {
        api.post(`/proctoring/session/${proctoringSessionId.value}/skill/exit`, {
            skill_id: skillId
        }).catch(e => {
    ...
    try {
        ...
        if (res.data.questions?.length > 0) {
            ...
            if (proctoringSessionId.value && res.data.skill?.id) {
                api.post(`/proctoring/session/${proctoringSessionId.value}/skill`, {
                    skill_id: res.data.skill.id,
                }).catch(e => console.warn('Failed to record skill entry:', e))
            }
                ...
                <div class="max-w-[1600px] w-full mx-auto flex justify-between items-center py-2 md:py-0 gap-1.5">
                    ...
                    <div class="flex items-center gap-1 sm:gap-2 md:gap-3 flex-wrap sm:flex-nowrap justify-end">
                        ...
                        <div v-if="!isDemo && timerConfig && timerConfig.skillDuration > 0" :class="[
                            ...
                            <span class="text-[10px] sm:text-xs md:text-lg font-black tabular-nums tracking-tighter">
                                {{ formattedTime }}
                            </span>
                        </div>
