...
const createCertificate = () => {
    showModal({
        ...
        message: 'Are you sure you want to create a certificate for this student? The admin will need to make it visible to the student.',
        type: 'info',
        showCancel: true,
        ...
        onConfirm: async () => {
            ...
            try {
                await api.post(`/partner/certificates/create-for-attempt/${attemptId}`);
                showModal({
                    title: 'Success',
        ...
        <div class="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700 pb-20 mt-6 px-4 md:px-12">
                ...
                <div class="flex items-center gap-4">
                    ...
                    <div>
                        <h1 class="text-3xl font-black text-slate-800 tracking-tight lowercase first-letter:uppercase">
                            Student Assessment Details</h1>
                        <p class="text-[10px] font-black text-slate-300 uppercase tracking-widest mt-1">Detailed
                            performance breakdown</p>
                    </div>
                </div>
                <div v-if="selectedAttempt" class="flex items-center gap-3">
            ...
            <div v-else-if="selectedAttempt" class="space-y-12">
                ...
                <div
                    ...
                    <div class="space-y-2">
                        <p class="text-[10px] font-black text-indigo-400 uppercase tracking-[0.4em] mb-1">Student
                            Profile</p>
                        ...
                        <p class="text-[10px] font-bold text-slate-400 tracking-widest">{{
                            selectedAttempt.student?.student_code || 'STU-XXXXXX' }}</p>
                    </div>
                    ...
                    <div class="space-y-2">
                        ...
                        <div class="flex items-baseline gap-2">
                            <span class="text-5xl font-black italic tracking-tighter text-emerald-400">
                                {{ Math.round(Number(getTotalScore(selectedAttempt)) / (selectedAttempt.skills_count ||
                                1), 2) }}
                            </span>
                            <span class="text-xl font-black text-slate-500"> / {{ Number((selectedAttempt.total_levels
                                || 1) * 100 / (selectedAttempt.skills_count || 1), 2) }} </span>
                        </div>
                                            ...
                                            <div class="flex flex-col md:flex-row items-start gap-8">
                                                ...
                                                <div class="grow space-y-6 w-full">
                                                    ...
                                                    <div class="grid grid-cols-1 gap-6 pt-6 border-t border-slate-50">
                                                        ...
                                                        <div v-if="answer.question?.type === 'ordering'"
                                                            class="space-y-4">
                                                            <p
                                                        ...
                                                        <div v-else-if="['drag_drop', 'fill_blank', 'matching', 'word_selection', 'click_word', 'highlight'].includes(answer.question?.type)"
                                                            class="space-y-4">
                                                            <p
                                                                class="text-[9px] font-black text-slate-400 uppercase tracking-[0.3em]">
