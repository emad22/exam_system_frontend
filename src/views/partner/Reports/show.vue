...
const createCertificate = async () => {
    const confirmed = await showConfirm(
        'Are you sure you want to create a certificate for this student? The admin will need to make it visible to the student.',
        'Create Certificate',
        'info',
    ...
    try {
        ...
        showAlert('Certificate created successfully.', 'Success', 'success');
    } catch (err) {
...
const fetchDetails = async () => {
    ...
    try {
        ...
        if (err.response?.status === 403) {
            await showAlert('Unauthorized to view this report.', 'Access Denied', 'danger');
